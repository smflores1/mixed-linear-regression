...
def test__initialize_parameters(test_data, linear_mixture):
    ...
    linear_model = utils.fit_linear_model(
    ...
    linear_mixture._initialize_parameters(
        test_data.X_mat, test_data.Y_mat, random_state
    )
    ...
    assert np.allclose(
        linear_mixture.slopes_, linear_model.linear_parameters.slope_tensor
    )
    assert np.allclose(
    ...
    for prob_xy_given_component_vec, X_vec, Y_vec in zip(
        ...
        for component_index, prob_xy_given_component_test in enumerate(
            ...
            res_vec = (
                Y_vec
                - np.dot(
                    X_vec, linear_model.linear_parameters.slope_tensor[component_index]
                )
            ...
            assert (
                abs(prob_xy_given_component_test - prob_xy_given_component_true) < 1e-10
            )
...
def test__estimate_log_prob_resp(test_data, linear_mixture):
    ...
    for weighted_log_prob_xy_vec, log_resp_vec, sum_comp_log_prob_xy in zip(
        weighted_log_prob_xy_mat,
        log_resp_mat,
        sum_comp_log_prob_xy_vec,
    ):

        assert sum_comp_log_prob_xy == special.logsumexp(weighted_log_prob_xy_vec)

        for weighted_log_prob_xy, log_resp in zip(
            weighted_log_prob_xy_vec, log_resp_vec
        ):

            assert log_resp == weighted_log_prob_xy - special.logsumexp(
                weighted_log_prob_xy_vec
            )
